...
import '../../ui/fire_door_web_shell_scaffold.dart';
...
class ProjectDrawingAccess {
  ...
  static String _bucketForLevel(String raw) {
    ...
    if (n.contains('level 1') || n == 'l1' || n == '1' || n.contains('first'))
      return 'Level 1';
    if (n.contains('level 2') || n == 'l2' || n == '2' || n.contains('second'))
      return 'Level 2';
    if (n.contains('basement') ||
        n == 'b' ||
        n == 'b1' ||
        n.contains('lower ground')) return 'Basement';
    return 'Other';
  }
...
class _DrawingViewerDialog extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final useFireDoorWebShell =
        kIsWeb && workspace == InspectionWorkspace.fireDoor;

    Widget wrappedBody(Widget child) {
      if (!useFireDoorWebShell) {
        return child;
      }
    ...
    if (drawing == null) {
      final missingBody = wrappedBody(
        const Center(child: Text('Drawing no longer exists.')),
      );

      if (useFireDoorWebShell) {
        return FireDoorWebShellScaffold(
          currentRoute:
              '/workspace/fire-door/inspection/projects/$surveyId/doors',
          title: fallbackTitle,
    ...
    final drawingBody = wrappedBody(
      ProjectDrawingAccess._isPdf(drawing)
