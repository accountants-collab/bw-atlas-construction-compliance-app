...
class FireDoorModuleProjectsScreen extends ConsumerStatefulWidget {
  const FireDoorModuleProjectsScreen({super.key});

  @override
  ConsumerState<FireDoorModuleProjectsScreen> createState() =>
      _FireDoorModuleProjectsScreenState();
}

class _FireDoorModuleProjectsScreenState
    extends ConsumerState<FireDoorModuleProjectsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (settings.activeWorkspaceKey != 'fire-door') {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ref
            ...
            .setActiveWorkspace('fire-door');
      });
    }

    final controller = ref.read(
        surveyControllerFamilyProvider(InspectionWorkspace.fireDoor).notifier);
    final state =
        ref.watch(surveyControllerFamilyProvider(InspectionWorkspace.fireDoor));
    final workerGroupId = isWorker
        ? ref
            .read(settingsControllerProvider.notifier)
            .workerGroupIdForWorkspace(
                workspaceKey: 'fire-door', userId: auth.uid)
        : null;

    final projects = state.surveys.where((s) {
      if (s.type != SurveyType.survey) return false;
    ...
    void addProject() {
      ...
      context
          .go('/workspace/fire-door/inspection/projects/${survey.id}/details');
    }
          ...
          child: Column(
            children: [
              const Align(
                ...
                child: Text(
                  'Create and manage projects for Fire Door Inspection.',
                              ...
                              child: ListTile(
                                ...
                                trailing: Row(
                                  ...
                                  children: [
                                    ...
                                    IconButton(
                                      ...
                                      onPressed: () {
                                        context.go(
                                            '/workspace/fire-door/inspection/projects/${p.id}/details');
                                      },
                                    ),
                                ...
                                onTap: () {
                                  if (p.isArchived) {
                                    ...
                                    context.go(
                                        '/workspace/fire-door/inspection/projects/${p.id}/doors');
                                  }
                                },
                              ),
                            ),
    ...
    if (kIsWeb) {
      return FireDoorWebShellScaffold(
        currentRoute: '/workspace/fire-door/inspection/projects',
