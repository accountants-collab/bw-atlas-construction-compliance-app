...
class _ProjectDetailsScreenState extends ConsumerState<ProjectDetailsScreen> {
  ...
  Future<void> _showExistingProjectsDialog() async {
    final workspaceKey = InspectionWorkspace.fireDoor;
    final surveyCtrl =
  ...
  Future<void> _showAssignExistingMemberDialog({
    ...
    try {
      ...
      debugPrint('Sort error (fire door project details): $e');
    }
  ...
  Widget _sectionCard({
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: isOptional ? Colors.grey.shade100 : Colors.grey.shade200),
      ),
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(icon,
                  size: 18, color: isOptional ? Colors.grey.shade500 : _accent),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
                  title,
                  style: TextStyle(
                    fontWeight: isOptional ? FontWeight.w700 : FontWeight.w900,
                    fontSize: isOptional ? 13 : 15,
                    color: isOptional ? Colors.grey.shade700 : _accent,
                  ),
                ),
              ),
  ...
  Future<bool> _ensureFireDoorDisclaimer({
    required Survey survey,
    required SurveyController controller,
    required String companyId,
  }) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content:
            Text('Declaration form must be completed before accessing doors.'),
  ...
  Widget build(BuildContext context) {
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Link an existing project from Fire Stopping, Snagging, or another module.',
                            style: TextStyle(
                ...
                child: FilledButton.icon(
                  ...
                  onPressed: () async {
                    ...
                    switch (widget.moduleKey) {
                      ...
                      case 'fire-stopping':
                      case 'snagging':
                        // Check disclaimer before accessing doors
                        if (companyId != null && companyId.isNotEmpty) {
                          final allowed = await _ensureFireDoorDisclaimer(
                            survey: survey,
                            controller: controller,
                            companyId: companyId,
                          );
    ...
    if (kIsWeb && widget.workspaceKey == 'fire-door') {
      return FireDoorWebShellScaffold(
        currentRoute:
            '/workspace/fire-door/inspection/projects/${widget.surveyId}/details',
        title: isFireStopping ? 'Project Details' : 'Project Details',
