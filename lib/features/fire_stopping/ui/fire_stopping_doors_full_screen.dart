...
class _DoorsScreenState extends ConsumerState<DoorsScreen> {
  ...
  Future<bool> _ensureInspectionDisclaimer({
  ...
  String _doorStatusLabel(Door d) {
    if (d.issues.isNotEmpty) return 'FAIL';
    if (d.result == DoorResult.pass) return 'PASS';
    if (d.result == DoorResult.fail || d.result == DoorResult.advisory) {
      return 'FAIL';
    }
    return 'NOT STARTED';
  }

  int _inspectionPhotoCount(Door d) {
    final seenIds = <String>{};
    var count = 0;

    void addPhotos(List<PhotoAttachment> photos) {
      for (final photo in photos) {
        if (photo.mimeType.startsWith('video/')) continue;
    ...
    addPhotos(d.doorPhotos);
    for (final result in d.inspectionResults.values) {
      addPhotos(result.photos);
    }
    for (final item in d.remedialItems) {
      addPhotos(item.originalInspectionPhotos);
    }
        ...
        builder: (ctx) => AlertDialog(
          ...
          actions: [
            ...
            FilledButton(
                ...
                child: const Text('Upload Drawing')),
          ],
        ),
      );
      if (uploadNow == true) {
  ...
  String _pinToken(Door door) {
    final pin = door.doorIdTag.trim();
    if (pin.isNotEmpty) {
      return pin;
    }
    return 'PIN-${door.number.toString().padLeft(3, '0')}';
  }

  String _singleDoorPdfFileName(
      Survey survey, Door door, AppSettings settings) {
    final _ = settings;
    return _sanitizeFileName(
        'FireStopping_${_dateTag(survey.reportDate)}_${_pinToken(door)}.pdf');
  }

  String _combinedPdfFileName(Survey survey, AppSettings settings) {
    final _ = settings;
    return _sanitizeFileName(
        'FireStopping_${_dateTag(survey.reportDate)}_AllPins.pdf');
  }

  String _zipExportFileName(Survey survey, AppSettings settings) {
    final _ = settings;
    return _sanitizeFileName(
        'FireStopping_${_dateTag(survey.reportDate)}_AllPins.zip');
  }
  ...
  Future<void> _showProjectExportDialog({
        ...
        child: Column(
          ...
          children: [
            ListTile(
              ...
              title:
                  Text(isFireStopping ? 'Single Item PDF' : 'Single Door PDF'),
              subtitle: Text(
                isFireStopping
            ...
            ListTile(
              ...
              title: Text(isFireStopping
                  ? 'All Items — Combined PDF'
                  : 'All Doors — Combined PDF'),
              subtitle: Text(
                isFireStopping
            ...
            ListTile(
              ...
              title: Text(isFireStopping
                  ? 'All Items — ZIP (one PDF per item)'
                  : 'All Doors — ZIP (one PDF per door)'),
              subtitle: Text(
                isFireStopping
    ...
    if (type == _ExportType.singleDoor) {
          ...
          child: Column(
            ...
            children: [
              const Padding(
                ...
                child: Text('Choose record to export',
                    style:
      ...
      await _exportPdf(
        ...
        settings: settings,
        fileName: _singleDoorPdfFileName(survey, chosenDoor, settings),
    ...
    try {
      ...
      if (action == _ExportAction.download) {
        if (kIsWeb) {
          downloadBytesWeb(
              bytes: zipBytes, fileName: zipName, mimeType: 'application/zip');
        } else {
  ...
  Future<void> _showApproveDialog({
    ...
    final numberCtrl =
        TextEditingController(text: door.approvedMaintainerNumber);
          ...
          FilledButton.icon(
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                    content: Text(
                        isFireStopping ? 'Item approved.' : 'Door approved.')),
              );
            },
  ...
  ({Color bg, Color border, Color text}) _fireStoppingStatusColors(
      String label) {
    switch (label) {
      case 'Completed':
        return (
  ...
  Widget build(BuildContext context) {
    ...
    final reportDetailsCompleted = survey.addressLine1.trim().isNotEmpty ||
        survey.siteAddress.trim().isNotEmpty;
    final isFireStopping = survey.type == SurveyType.fireStopping;
    final doors = survey.doors;

    bool canDeleteDoorOrItem(Door door) {
    ...
    Future<void> openActionsSheet({required String doorId}) async {
      final isFireStopping = survey.type == SurveyType.fireStopping;
      final selectedDoor =
          controller.getDoorById(surveyId: widget.surveyId, doorId: doorId);
      if (selectedDoor == null) return;

      final canDelete = canDeleteDoorOrItem(selectedDoor);
      final action = await showModalBottomSheet<_DoorAction>(
        context: context,
        showDragHandle: true,
        builder: (ctx) => _DoorActionsSheet(
          ...
          if (!canDelete) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  isFireStopping
                      ? 'Only managers and admins can delete items.'
                      : 'Only managers and admins can delete doors.',
                ),
              ),
            );
            return;
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: FilledButton.tonalIcon(
                    onPressed: () => _showProjectExportDialog(
                        context: context, survey: survey, settings: settings),
                            ...
                            try {
                              ...
                              if (project == null) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                      content: Text(
                                          'Project not found. Please refresh project list.')),
                                );
                                return;
                              }

                              final allowed = await _ensureInspectionDisclaimer(
                                survey: project,
                                controller: controller,
                              );
                              if (!allowed) return;
                              ...
                              if (updatedSurvey == null ||
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                      content: Text(
                                          'Could not create report item. Try again.')),
                                );
                                return;
                              }
            ...
            for (final d in doors)
              Builder(builder: (context) {
                final fireStoppingStatus =
                    isFireStopping ? _fireStoppingStatusLabel(d) : '';
                final fireStoppingStatusColors = isFireStopping
                    ? _fireStoppingStatusColors(fireStoppingStatus)
                    : null;
                final doorStatus = isFireStopping ? '' : _doorStatusLabel(d);
                final photoCount = isFireStopping
                    ? d.doorPhotos.length
                    : _inspectionPhotoCount(d);
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Text([
                      if (d.floor.trim().isNotEmpty)
                        '${isFireStopping ? 'Room / Area' : 'Level'}: ${d.floor}',
                      if (d.area.trim().isNotEmpty)
                        '${isFireStopping ? 'Exact Location' : 'Location'}: ${d.area}',
                      if (isFireStopping) 'Defects: $fireStoppingDefectCount',
                      'Status: ${isFireStopping ? fireStoppingStatus : doorStatus}',
                      'Photos: $photoCount',
                    ...
                    onTap: () => context.go(
                        '${widget.routePrefix}/${widget.surveyId}/doors/${d.id}'),
    ...
    if (kIsWeb) {
      return FireDoorWebShellScaffold(
        currentRoute: '${widget.routePrefix}/${widget.surveyId}/doors',
...
class _DoorActionsSheet extends StatelessWidget {
  ...
  const _DoorActionsSheet({this.isFireStopping = false, this.canDelete = true});
...
class _DisclaimerResult {
  ...
  const _DisclaimerResult(
      {required this.accepted, required this.inspectorName});
}

class _DisclaimerSheet extends StatefulWidget {
  final bool isFireStopping = true;
                      ...
                      children: [
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            ...
                            errorText: _showValidation && !hasName
                    ...
                    child: FilledButton(
                      onPressed: () {
                        ...
                        Navigator.pop(
                          context,
                          _DisclaimerResult(
                              accepted: true, inspectorName: _name.text.trim()),
                        );
                      },
...
6) Urgent defects
Where failed defects are identified, immediate mitigation and/or repair may be required. This report does not constitute permission to keep a non-compliant door in service.
''';
