...
import '../domain/drawing_pin_reference.dart';
...
class SurveyPdfBuilder {
  ...
  static pw.Widget _clientHeaderSummarySection({
    ...
    final fireStoppingDefects = _enteredFireStoppingDefects(door);
    final fireStoppingFindings = fireStoppingDefects.length;
    final fireStoppingActions = fireStoppingDefects
        .where((defect) => _cleanText(defect.recommendedAction).isNotEmpty)
        .length;
    ...
    final exactLocation = _cleanText(door.area);
        ...
        children: [
          pw.Expanded(
            child: pw.Table(
              ...
              children: [
                ...
                if (!isFireStopping || exactLocation.isNotEmpty)
                  _compactRow(
          ...
          pw.Expanded(
            child: pw.Table(
              ...
              children: [
                _compactRow(isFireStopping ? 'Room / Area' : 'Floor / Level',
                    _safe(door.floor)),
                _compactRow(
                  isFireStopping ? 'Item Findings' : 'Non-Conformities',
                  isFireStopping
                      ? fireStoppingFindings.toString()
                      : failEntries.length.toString(),
                ),
                _compactRow(
                  isFireStopping ? 'Recommended Actions' : 'Remedial Actions',
                  isFireStopping
                      ? fireStoppingActions.toString()
                ...
                if (!isFireStopping)
                  _compactRow(
                    survey.type == SurveyType.snagging
                        ...
                        : 'Door Pin',
                    pinLabel.isEmpty
                        ? (reportReference.isEmpty ? '-' : reportReference)
                        : _safe(pinLabel),
                  ),
              ],
            ),
  ...
  static pw.Widget _assetInformationSection(
    ...
    if (isFireStopping) {
      ...
      final exactLocation = _cleanText(door.area);
      final fireStoppingRows = <pw.TableRow>[
  ...
  static pw.Widget _fireStoppingItemContentSection({
    ...
    final defects = _enteredFireStoppingDefects(door);
    final statusLabel = _fireStoppingStatusLabel(door);
      ...
      child: pw.Column(
        ...
        children: [
          ...
          if (drawingReference != null) ...[
            ...
            pw.SizedBox(height: 6),
            _fireStoppingManualLocationEvidenceWidget(
                survey: survey, door: door),
          ],
        ],
      ),
    );
  ...
  static pw.Widget _fireStoppingManualLocationEvidenceWidget({
    required Survey survey,
    required Door door,
  }) {
    final itemRef = _safeDoorRef(door);
    final roomArea = _safe(door.floor);
    final exactLocation = _cleanText(door.area);
    final hasDrawing = survey.projectDrawings.isNotEmpty;
      ...
      child: pw.Column(
        ...
        children: [
          pw.Text(
            hasDrawing
                ? _cleanText(
                    'No pin was linked for this item. Manual location reference is recorded below.')
                : _cleanText(
                    'No drawing was available for this project. Manual location reference is recorded below.'),
          ...
          pw.Table(
            ...
            children: [
              _compactRow('Item Reference', itemRef),
              _compactRow('Room / Area', roomArea),
              if (exactLocation.isNotEmpty)
                _compactRow('Exact Location', _safe(exactLocation)),
            ],
          ),
        ],
      ),
  ...
  static pw.Widget _reportSummaryCard(Survey survey) {
    ...
    for (final door in survey.doors) {
      ...
      final hasCritical = door.inspectionResults.values
          .any((r) => r.outcome == InspectionOutcome.criticalFail);
      if (hasCritical) {
        critical += 1;
      }
    }
  ...
  static pw.Widget _doorMaintenanceTableSection(
    ...
    final advisoryEntries = answeredEntries
        .where((entry) => entry.value.outcome == InspectionOutcome.advisory)
        .toList();
      ...
      child: pw.Table(
        ...
        children: [
          ...
          ...findingEntries.map((entry) {
            ...
            final isFail = result.outcome == InspectionOutcome.fail ||
  ...
  static pw.Widget _declarationAndSignatureCard({
    ...
    final signatureDate = _resolveSignatureDateTime(
        ...
        children: [
          ...
          pw.Expanded(
            ...
            child: pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.stretch,
              children: [
                pw.Text('Inspector Initials',
                    style: _style(7.4, bold: true, color: _secondaryText)),
                pw.SizedBox(height: 3),
                _signatureCard(survey: survey, approval: approval, door: door),
                pw.SizedBox(height: 4),
                pw.Text(
                  'Date: ${_date(signatureDate)}   Time: ${_fmtTime(signatureDate)}',
                  style: _style(7.2, color: _secondaryText),
                ),
              ],
            ),
  ...
  static pw.Widget _signatureCard(
    ...
    if (approval != null && approval.signatureImageBytes.isNotEmpty) {
      final signatureImage =
          _memoryImageFromBytes(approval.signatureImageBytes);
      if (signatureImage != null) {
        return pw.Container(
          height: 48,
    ...
    final typedSignatureName = _inspectorName(survey, approval, door);
    final hasInspectorName = _cleanText(typedSignatureName).isNotEmpty &&
        _cleanText(typedSignatureName).toLowerCase() != 'not provided';
    final typedSignatureInitials = _signatureInitials(typedSignatureName);
        ...
        child: pw.Column(
          ...
          children: [
            pw.Text(
              ...
              style: _style(15.0, bold: true, color: _text),
            ),
            pw.SizedBox(height: 2),
            pw.Text(typedSignatureName,
    ...
    return pw.Container(
      ...
      child: pw.Text('Initials not provided',
          style: _style(7.0, bold: true, color: _fail)),
    );
  }
  ...
  static Future<pw.Widget?> _fireStoppingDrawingReferenceWidget(
    ...
    if (survey.type == SurveyType.fireStopping) {
      final linkedRef = DrawingPinReference.fromMetadata(
          _cleanText(door.fireStoppingItemType));
      if (linkedRef != null) {
        metaDrawingId = linkedRef.drawingId;
        metaPinId = linkedRef.pinId;
      }
    }
  ...
  static String _composeFireStoppingDefectDescription(
    ...
    final penetrationType = _cleanText(defect.serviceType);
    ...
    final descriptors = <String>[
      if (penetrationType.isNotEmpty) 'Penetration: $penetrationType',
      if (template.isNotEmpty) 'Defect: $template',
    ];
    final typedSummary = descriptors.join(' | ');
    final mainText = description.isNotEmpty ? description : typedSummary;
  ...
  static String _fireStoppingNotes(Door door) {
    ...
    if (notes.isNotEmpty &&
        notes.toLowerCase() !=
            _cleanText(firstDefect?.template ?? '').toLowerCase()) {
      return notes;
    }
  ...
  static List<FireStoppingDefect> _enteredFireStoppingDefects(Door door) {
    final source = door.fireStoppingDefects.isNotEmpty
        ? door.fireStoppingDefects
        : [
            FireStoppingDefect(
              id: 'legacy',
              template: door.fireStoppingDefectDescription,
    ...
    return source.where((defect) {
      return _cleanText(defect.serviceType).isNotEmpty ||
          _cleanText(defect.template).isNotEmpty ||
          _cleanText(defect.description).isNotEmpty ||
          _cleanText(defect.recommendedAction).isNotEmpty ||
          _cleanText(defect.fireRating).isNotEmpty ||
          _cleanText(defect.lengthMm).isNotEmpty ||
          _cleanText(defect.widthMm).isNotEmpty ||
          defect.photos.isNotEmpty;
    }).toList();
  }

  static FireStoppingDefect? _primaryFireStoppingDefect(Door door) {
    final entered = _enteredFireStoppingDefects(door);
  ...
  static String _fireStoppingPinLabel({Survey? survey, required Door door}) {
    ...
    if (survey?.type == SurveyType.fireStopping) {
      final linkedRef = DrawingPinReference.fromMetadata(
          _cleanText(door.fireStoppingItemType));
      if (linkedRef != null) {
        drawingId = linkedRef.drawingId;
        pinId = linkedRef.pinId;
      }
    }
    if (pinId.isEmpty) {
  ...
  static String _signatureInitials(String fullName) {
    final parts = _cleanText(fullName)
        .split(RegExp(r'\s+'))
    ...
    final secondInitial =
        (parts.length >= 2 ? parts.last[0] : parts.first[0]).toUpperCase();
    return '$firstInitial.$secondInitial';
  }

  static DateTime _resolveSignatureDateTime({
    required String inspectorName,
    required Approval? approval,
    required Door door,
  }) {
    if (approval?.approvedDate != null) {
      return approval!.approvedDate;
    }
    if (door.approvedAt != null) {
      return door.approvedAt!;
    }

    final hasInspectorName = _cleanText(inspectorName).isNotEmpty &&
        _cleanText(inspectorName).toLowerCase() != 'not provided';
    if (hasInspectorName) {
      return door.inspectionDate;
    }
    return DateTime.now();
  }
