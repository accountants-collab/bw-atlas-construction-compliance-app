...
class ProjectDrawingAccess {
  ...
  static Future<DrawingPinSelectionResult?> showDrawingPicker({
    ...
    if (survey.projectDrawings.length == 1) {
      final single = survey.projectDrawings.first;
      final resolved = beforeOpen == null ? single : await beforeOpen(single);
...
class _DrawingViewerDialog extends ConsumerWidget {
  ...
  Survey? _getSurveyForWorkspace(WidgetRef ref, InspectionWorkspace workspace) {
    final controller =
        ref.read(surveyControllerFamilyProvider(workspace).notifier);
    return controller.getById(surveyId);
  }

  ({Survey survey, InspectionWorkspace workspace})? _resolveSurveyAndWorkspace(
      WidgetRef ref) {
    if (workspaceOverride != null) {
      ref.watch(surveyControllerFamilyProvider(workspaceOverride!));
      final survey = _getSurveyForWorkspace(ref, workspaceOverride!);
      if (survey != null) {
    ...
    for (final ws in InspectionWorkspace.values) {
      ref.watch(surveyControllerFamilyProvider(ws));
      final survey = _getSurveyForWorkspace(ref, ws);
      if (survey != null) {
...
class _PinnedPdfDrawingCanvasState
  ...
  Widget build(BuildContext context) {
    ...
    final isSelectionMode = widget.selectionConfig?.enablePinPlacement ?? false;
    final canManagePins = auth.userRole == UserRole.manager ||
        isSelectionMode ||
        widget.isFireStopping;

    ref.watch(surveyDrawingPinsRevisionProvider(widget.workspace));
    ref.watch(surveyControllerFamilyProvider(widget.workspace));
    final controller =
        ref.read(surveyControllerFamilyProvider(widget.workspace).notifier);
    final survey = controller.getById(widget.surveyId);
    final drawingMatches = survey?.projectDrawings
            .where((d) => d.id == widget.drawing.id)
            .toList() ??
        const <ProjectDrawing>[];
    final currentDrawing =
        drawingMatches.isEmpty ? widget.drawing : drawingMatches.first;
    final allDrawingPins = controller.getDrawingPins(
      surveyId: widget.surveyId,
      drawingId: widget.drawing.id,
    ...
    if (widget.selectionConfig?.hideOtherPins ?? false) {
      pagePins = highlightedPinId.isEmpty
          ? <FloorPlanPin>[]
          : pagePins.where((p) => p.id == highlightedPinId).toList();
    }
