...
class SurveyPdfBuilder {
  ...
  static Future<void> _addDoorReport({
    ...
    final drawingReference = await _fireStoppingDrawingReferenceWidget(survey: survey, door: door);

    if (isFireStopping) {
      content
        ..add(pw.SizedBox(height: _sectionGap))
        ..add(_fireStoppingItemContentSection(door: door, drawingReference: drawingReference));
    } else {
      ...
      // Add Fire Door replacement widget if needed
      final replacementWidget = _replacementInformationWidget(door);
      if (replacementWidget != null) {
        content
          ..add(pw.SizedBox(height: _sectionGap))
          ..add(replacementWidget);
      }
    ...
    content
      ...
      ..add(_inspectionDisclaimerCard(isFireStopping: isFireStopping))
      ..add(pw.SizedBox(height: 6))
      ..add(_declarationAndSignatureCard(approval: approval, door: door));
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        footer: (context) => _pageFooter(
          ...
          generatedBy: _cleanText(generatedBy).isEmpty ? _inspectorName(survey, approval, door) : generatedBy,
    ...
    return pw.Column(
      ...
      children: [
        pw.Row(
          ...
          children: [
            pw.Container(
              constraints: pw.BoxConstraints(maxWidth: _contentWidth * 0.25, maxHeight: 44),
            ...
            pw.Container(
              ...
              child: pw.Column(
                ...
                children: [
                  pw.Text('Company', style: _style(7.0, bold: true, color: _secondaryText)),
                  pw.SizedBox(height: 2),
        ...
        children: [
          pw.Expanded(
            child: pw.Table(
              ...
              children: [
                ...
                _compactRow(isFireStopping ? 'Pin Number' : 'Door ID / Reference', _safeDoorRef(door)),
                _compactRow('Project / Development', _safeSurveyName(survey)),
                _compactRow(isFireStopping ? 'Exact Location' : 'Door location', _safe(door.area)),
              ],
            ),
          ),
          pw.SizedBox(width: 8),
          pw.Expanded(
            child: pw.Table(
              ...
              columnWidths: const {0: pw.FlexColumnWidth(1.1), 1: pw.FlexColumnWidth(1.4)},
              children: [
                _compactRow(isFireStopping ? 'Room / Area' : 'Floor / Level', _safe(door.floor)),
                _compactRow(isFireStopping ? 'Item Findings' : 'Non-Conformities', failEntries.length.toString()),
                _compactRow(isFireStopping ? 'Recommended Actions' : 'Remedial Actions', actionsRequired.toString()),
                _compactRow(
                  isFireStopping
                      ? 'PIN Number'
                      : (survey.type == SurveyType.snagging ? 'Snag Pin' : 'Door Pin'),
                  pinLabel.isEmpty
                      ? (isFireStopping ? '-' : (reportReference.isEmpty ? '-' : reportReference))
                      : _safe(pinLabel),
                ),
              ],
            ),
  ...
  static pw.Widget _assetInformationSection({required Survey survey, required Door door}) {
    final isFireStopping = survey.type == SurveyType.fireStopping;
    if (isFireStopping) {
      ...
      final acousticRating = _fireStoppingAcousticRating(door: door, defect: firstDefect);
      return _sectionCard(
      ...
      child: pw.Table(
        ...
        children: [
          pw.TableRow(children: [
            ...
            _tCell(_projectAddress(survey)),
            _tCellHdr('Door location'),
            _tCell(_safe(door.area)),
          ]),
          pw.TableRow(children: [
            _tCellHdr('Floor / Level'),
            _tCell(_safe(door.floor)),
  ...
  static pw.Widget _fireStoppingItemContentSection({
    ...
    final defects = door.fireStoppingDefects.isNotEmpty
        ...
        : [
          ...
          ].where((d) => d.description.trim().isNotEmpty || d.recommendedAction.trim().isNotEmpty).toList();
    final statusLabel = _fireStoppingStatusLabel(door);
              ...
              pw.Table(
                ...
                children: [
                  ...
                  for (var i = 0; i < defects.length; i++)
                    pw.TableRow(children: [
                      _tCell((i + 1).toString()),
                      _tCell(_safe(_composeFireStoppingDefectDescription(defects[i]))),
    ...
    return _sectionCard(
      ...
      child: pw.Table(
        ...
        children: [
          ...
          pw.TableRow(children: [
            _tCellHdr('Maintainer Number'),
            _tCell(_safe(door.approvedMaintainerNumber)),
            _tCellHdr('Company'),
  ...
  static pw.Widget _inspectionDisclaimerCard({required bool isFireStopping}) {
    return _sectionCard(
      title: isFireStopping ? 'Fire Stopping Inspection Disclaimer' : 'Inspection Disclaimer',
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.stretch,
        children: [
          pw.Text(
            isFireStopping
                ? 'This fire stopping report is based on a visual, non-intrusive inspection of accessible areas only at the time of inspection.'
                : 'This report is based on a visual, non-intrusive inspection of accessible areas only at the time of inspection.',
            style: _style(7.8, color: _text),
          ),
          pw.SizedBox(height: 3),
          pw.Text(
            isFireStopping
                ? 'Concealed voids, substrate defects, or non-visible penetration details may exist and are outside the scope unless intrusive investigation is explicitly commissioned.'
                : 'Hidden defects may exist and no destructive testing, laboratory analysis, or certification verification is included unless explicitly stated.',
            style: _style(7.8, color: _text),
          ),
        ],
      ),
  ...
  static List<MapEntry<String, InspectionCheckResult>> _failedEntries(Door door) {
  ...
  static pw.Widget _pageFooter({
    ...
    final by = _cleanText(generatedBy).isEmpty ? 'System' : _cleanText(generatedBy);
    return pw.Column(
          ...
          children: [
            pw.Expanded(
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  pw.Text(_cleanText(companyName), style: _style(7.0, color: _secondaryText)),
                  pw.Text('Register Ref: ${_cleanText(registerReference)}', style: _style(6.5, color: _secondaryText)),
                  pw.Text('Generated on: ${_dateTime(generatedAt)}', style: _style(6.5, color: _secondaryText)),
                  pw.Text('Generated by: $by', style: _style(6.5, color: _secondaryText)),
                ],
              ),
            ),
            pw.Expanded(
              child: pw.Column(
                ...
                children: [
                  pw.Text(
                    'RMA 058 blank - Record of Maintenance Activities - Iss 4 010123',
                    textAlign: pw.TextAlign.center,
  ...
  static pw.Widget _siteDetailsCard({
    ...
    final rows = <List<String>>[
      ...
      ['Project Number', reportReference.isEmpty ? 'Not provided' : reportReference],
    ];
    return _sectionCard(
  ...
  static pw.Widget _reportSummaryCard(Survey survey) {
    ...
    for (final door in survey.doors) {
      ...
      final hasCritical = door.inspectionResults.values.any((r) => r.outcome == InspectionOutcome.criticalFail);
      if (hasCritical) {
        critical += 1;
      }
    }
    return _sectionCard(
      ...
      child: _detailGrid([
        ...
        ['Critical Fail', critical.toString()],
      ], columns: 2),
    );
  }

  static pw.Widget _doorMaintenanceTableSection({required Door door, required Approval? approval}) {
    final answeredEntries = door.inspectionResults.entries.where((entry) {
      final outcome = entry.value.outcome;
      return outcome != InspectionOutcome.notAnswered && outcome != InspectionOutcome.notApplicable;
    }).toList();

    final failEntries = answeredEntries.where((entry) {
      final outcome = entry.value.outcome;
      return outcome == InspectionOutcome.fail || outcome == InspectionOutcome.criticalFail;
    }).toList();

    final advisoryEntries = answeredEntries.where((entry) => entry.value.outcome == InspectionOutcome.advisory).toList();
    final passCount = answeredEntries.where((entry) => entry.value.outcome == InspectionOutcome.pass).length;

    if (answeredEntries.isEmpty) {
      return _sectionCard(
        title: 'INSPECTION FINDINGS',
        child: pw.Text('Inspection incomplete', style: _style(9, bold: true, color: _secondaryText)),
      );
    }

    if (failEntries.isEmpty && advisoryEntries.isEmpty) {
      return _sectionCard(
        title: 'INSPECTION FINDINGS',
        child: pw.Text(
    ...
    final findingEntries = [...failEntries, ...advisoryEntries];
      ...
      child: pw.Table(
        ...
        children: [
          ...
          ...findingEntries.map((entry) {
            ...
            final isFail = result.outcome == InspectionOutcome.fail || result.outcome == InspectionOutcome.criticalFail;
            final isCritical = result.outcome == InspectionOutcome.criticalFail;
            final isAdvisory = result.outcome == InspectionOutcome.advisory;
            final artCode = isFail ? check?.artCodeOnFail : null;
            final remedial = _findRemedialForIssue(door: door, issueId: entry.key, checkTitle: check?.title ?? '');
            final prefix = isCritical
                ? '[CRITICAL] '
                : isFail
                    ? '[FAIL] '
                    : (isAdvisory ? '[ADVISORY] ' : '');
            final description = _cleanText('$prefix${check?.title ?? entry.key}');
            ...
            return pw.TableRow(
              children: [
                ...
                _tableCell(artCode == null ? 'Not required' : 'ART${artCode.toString().padLeft(2, '0')}'),
                _tableCell(remedial != null && remedial.status == RemedialStatus.approved ? 'Yes' : 'No'),
                _tableCell(_nullableDate(remedial?.approvedAt ?? remedial?.completedDate)),
                _tableCell(_cleanText(remedial?.approvedBy ?? remedial?.completedBy ?? door.approvedMaintainerName)),
                _tableCell(_nullableDate(approval?.nextMaintenanceDueDate)),
                _tableCell(overrideComment.isNotEmpty ? overrideComment : fallbackRecommendation),
              ],
            );
          }),
        ],
  ...
  static pw.Widget _declarationAndSignatureCard({required Approval? approval, required Door door}) {
        ...
        children: [
          ...
          pw.Expanded(
            ...
            child: pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.stretch,
              children: [
                pw.Text('Signature', style: _style(7.4, bold: true, color: _secondaryText)),
                pw.SizedBox(height: 3),
                _signatureCard(approval, door),
                pw.SizedBox(height: 4),
                pw.Text('Date: ${_nullableDate(signatureDate)}', style: _style(7.2, color: _secondaryText)),
              ],
            ),
          ),
        ],
  ...
  static pw.Widget _signatureCard(Approval? approval, Door door) {
    if (approval != null && approval.signatureImageBytes.isNotEmpty) {
      final signatureImage = _memoryImageFromBytes(approval.signatureImageBytes);
    ...
    final typedSignatureName = _cleanText(
      door.approvedMaintainerName.isNotEmpty
          ? door.approvedMaintainerName
          : (approval?.approvedBy ?? ''),
    );
    final typedSignatureId = _cleanText(door.approvedMaintainerNumber);

    if (typedSignatureName.isNotEmpty) {
      return pw.Container(
        height: 48,
        ...
        child: pw.Column(
          ...
          children: [
            pw.Text(
              typedSignatureName,
              style: _style(9.6, bold: true, color: _text),
            ),
            if (typedSignatureId.isNotEmpty) ...[
              pw.SizedBox(height: 2),
              pw.Text("Maintainer no: $typedSignatureId", style: _style(6.9, color: _secondaryText)),
            ],
          ],
        ),
      );
    ...
    return pw.Container(
      ...
      child: pw.Text('Signature not provided', style: _style(7.0, bold: true, color: _fail)),
    );
  }

  static pw.Widget _doorSummaryCard({required Survey survey, required Door door}) {
    ...
    final rows = <List<String>>[
      ...
      ['Certification status', _doorCertificationStatusLabel(door.classification)],
  ...
  static pw.Widget _inspectionResultsSection(Door door) {
    ...
    for (final check in inspectionChecks) {
      ...
      categories.putIfAbsent(sectionTitle, () => <InspectionCheckDefinition>[]).add(check);
    }
    ...
    for (final entry in categories.entries) {
      ...
      for (final check in entry.value) {
        ...
        if (outcome == InspectionOutcome.criticalFail) hasCritical = true;
        if (outcome == InspectionOutcome.fail) hasFail = true;
        if (outcome == InspectionOutcome.advisory) hasAdvisory = true;
    ...
    return _sectionCard(
      ...
      child: pw.Table(
        border: pw.TableBorder.all(color: _softLine, width: 0.6),
        columnWidths: const {0: pw.FlexColumnWidth(2.8), 1: pw.FlexColumnWidth(1.2)},
        children: [
          pw.TableRow(
            decoration: pw.BoxDecoration(color: _sectionTint),
            children: [
      ...
      child: pw.Table(
        ...
        children: [
          ...
          ...commentedEntries.map((entry) {
            ...
            final isCritical = result.outcome == InspectionOutcome.criticalFail;
            final outcomeLabel = isCritical ? 'CRITICAL FAIL' : 'FAIL';
  ...
  static pw.Widget? _recommendedActionsSummaryWidget(Door door) {
    ...
    final codePattern = RegExp(r'^(ART\d{2}[a-zA-Z]*(?:-custom)?)\s*(?:-|:)?\s*(.+)$', caseSensitive: false);
    return _sectionCard(
  ...
  static pw.Widget? _videoEvidenceWidget({required Survey survey, required Door door}) {
    final isFireStopping = survey.type == SurveyType.fireStopping;
    if (isFireStopping) {
      return null;
    }
    ...
    for (final media in door.doorPhotos) {
      ...
      final name = media.fileName.trim().isEmpty ? 'video_file' : media.fileName.trim();
  ...
  static List<pw.Widget> _photoEvidenceWidgets({required Survey survey, required Door door}) {
  ...
  static pw.Widget _photoLayout(List<({List<int> bytes, String caption})> photos) {
    if (photos.length == 1) {
  ...
  static pw.Widget _sectionHeader(String title) {
    return pw.Container(
      padding: const pw.EdgeInsets.symmetric(horizontal: _sectionPadding, vertical: 7),
      decoration: pw.BoxDecoration(
        color: _panelAlt,
        border: pw.Border.all(color: _softLine),
      ...
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.stretch,
        children: [
          pw.Text(_cleanText(title), style: _style(10.2, bold: true, color: _text)),
          pw.SizedBox(height: 3),
          pw.Container(height: 1, color: _softLine),
        ],
      ),
  ...
  static List<({List<int> bytes, String caption})> _gatherPhotoTiles({required Survey survey, required Door door}) {
    ...
    if (isFireStopping) {
      ...
      for (var i = 0; i < photoBytes.length; i++) {
        result.add((bytes: photoBytes[i], caption: _cleanText('Pin Photo ${i + 1} of ${photoBytes.length}')));
      }
      return result;
    }
    ...
    for (final entry in door.inspectionResults.entries) {
      ...
      if (outcome == InspectionOutcome.notAnswered || outcome == InspectionOutcome.notApplicable) {
        continue;
      }
      ...
      final check = _findCheck(entry.key);
      final isFail = outcome == InspectionOutcome.fail || outcome == InspectionOutcome.criticalFail;
      final artCode = isFail ? check?.artCodeOnFail : null;
      ...
      final outcomeLabel = outcome == InspectionOutcome.pass
          ...
          : outcome == InspectionOutcome.advisory
              ...
              : outcome == InspectionOutcome.fail
                  ? 'FAIL'
                  : (outcome == InspectionOutcome.criticalFail ? 'CRITICAL' : 'INSPECTION');
      final artLabel = artCode == null ? '' : 'ART${artCode.toString().padLeft(2, '0')} - ';
      for (final photo in inspection.photos) {
        final normalized = _normalizePhotoBytes(photo.bytes);
        if (normalized == null) continue;
        final key = _photoFingerprint(normalized);
        if (!seen.add(key)) continue;
        result.add((bytes: normalized, caption: _cleanText('$outcomeLabel - $artLabel$title')));
      }
    }
    ...
    for (final item in door.remedialItems) {
      for (final photo in item.originalInspectionPhotos) {
        ...
        final key = _photoFingerprint(normalized);
        if (!seen.add(key)) continue;
        result.add((bytes: normalized, caption: _cleanText('Before - ${item.title}')));
      }
      for (final photo in item.afterRepairPhotos) {
        ...
        final key = _photoFingerprint(normalized);
        if (!seen.add(key)) continue;
        result.add((bytes: normalized, caption: _cleanText('After - ${item.title}')));
      }
    }
    ...
    for (final photo in door.doorPhotos) {
      ...
      result.add((bytes: normalized, caption: _cleanText('$labelPrefix - ${_safeDoorRef(door)}')));
    }
  ...
  static bool _isVideoMedia(PhotoAttachment media) {
    ...
    return const ['mp4', 'mov', 'webm', 'm4v', 'avi', 'mkv', '3gp', 'flv', 'wmv', 'webm'].contains(ext);
  }

  static Future<pw.Widget?> _fireStoppingDrawingReferenceWidget({required Survey survey, required Door door}) async {
    ...
    final sourceBytes = await _drawingSourceBytesForPreview(drawing: matchedDrawing, page: matchedPin.page);
    if (sourceBytes == null) {
      return _sectionCard(
        title: 'Drawing Reference',
        child: pw.Text(
          _cleanText('Pin linked but drawing preview is unavailable for this file.'),
    ...
    final markerX = (matchedPin.x.clamp(0.0, 1.0) * full.width).round().clamp(0, full.width - 1);
    final markerY = (matchedPin.y.clamp(0.0, 1.0) * full.height).round().clamp(0, full.height - 1);
    ...
    img.fillCircle(snippet, x: localX, y: localY, radius: 11, color: img.ColorRgb8(198, 40, 40));
    img.drawCircle(snippet, x: localX, y: localY, radius: 18, color: img.ColorRgb8(255, 255, 255));
    img.drawCircle(snippet, x: localX, y: localY, radius: 19, color: img.ColorRgb8(255, 255, 255));
    ...
    final pinPrefix = survey.type == SurveyType.fireStopping
        ? 'Pin'
        : (survey.type == SurveyType.snagging ? 'Snag Pin' : 'Door Pin');
    final pinLabel = rawPinLabel.toLowerCase().startsWith('pin') ? rawPinLabel : '$pinPrefix $rawPinLabel';
    final title = survey.type == SurveyType.fireStopping
        ? 'Drawing Reference'
        : (survey.type == SurveyType.snagging ? 'Snagging Plan Reference' : 'Door Plan Reference');
  ...
  static Future<List<int>?> _drawingSourceBytesForPreview({
    ...
    final isPdf = drawing.mimeType.toLowerCase().contains('pdf') || drawing.fileName.toLowerCase().endsWith('.pdf');
    if (!isPdf) {
    ...
    try {
      ...
      await for (final raster in Printing.raster(Uint8List.fromList(drawing.bytes), pages: [targetPage], dpi: 240)) {
  ...
  static String _composeFireStoppingDefectDescription(FireStoppingDefect defect) {
    final template = _cleanText(defect.template);
    final description = _cleanText(defect.description);
    final dimensions = [
      if (_cleanText(defect.lengthMm).isNotEmpty) 'L: ${_cleanText(defect.lengthMm)} mm',
      if (_cleanText(defect.widthMm).isNotEmpty) 'W: ${_cleanText(defect.widthMm)} mm',
    ].join(', ');
  ...
  static String _fireStoppingAcousticRating({required Door door, required FireStoppingDefect? defect}) {
  ...
  static FireStoppingDefect? _primaryFireStoppingDefect(Door door) {
    if (door.fireStoppingDefects.isNotEmpty) return door.fireStoppingDefects.first;
    if (door.fireStoppingDefectDescription.trim().isEmpty && door.fireStoppingRecommendedAction.trim().isEmpty) {
      return null;
    }
      ...
      for (final drawing in survey.projectDrawings) {
        ...
        for (final pin in drawing.pins) {
          if (pinId.isNotEmpty && pin.id == pinId) {
            return _cleanText(pin.label.isNotEmpty ? pin.label : pin.doorNumber);
          }
        }
      }
    }
  ...
  static Approval? _latestApproval(Door door) {
    ...
    for (final item in door.remedialItems) {
      ...
      if (latest == null || approval.approvedDate.isAfter(latest.approvedDate)) {
        latest = approval;
      }
    }
    return latest;
  ...
  static String _registerReference(Survey survey, Approval? approval, Door door) {
    ...
    final override = approval == null ? '' : _cleanText(approval.certificateJobReferenceOverride);
    if (override.isNotEmpty) {
      return override;
    }
    final datePart = '${survey.createdAt.year}${survey.createdAt.month.toString().padLeft(2, '0')}${survey.createdAt.day.toString().padLeft(2, '0')}';
    return 'REG-$datePart-${door.number.toString().padLeft(3, '0')}';
  }
  ...
  static RemedialItem? _findRemedialForIssue({required Door door, required String issueId, required String checkTitle}) {
    for (final item in door.remedialItems) {
      if (_cleanText(item.issueId) == _cleanText(issueId)) {
        return item;
      }
      if (_cleanText(item.title).toLowerCase() == _cleanText(checkTitle).toLowerCase()) {
        return item;
      }
    }
    return null;
  ...
  static ({PdfColor color, PdfColor background}) _resultStyle(String status) {
    ...
    if (normalized == 'ADVISORY') {
      return (
        ...
        background: PdfColor.fromInt(0xFFFFF6E8),
      );
    }
    if (normalized == 'CRITICAL FAIL') {
      return (
        color: PdfColor.fromInt(0xFF7F1D1D),
        background: PdfColor.fromInt(0xFFFDE8E8),
      );
    }
    if (normalized == 'FAIL') {
  ...
  static String _overallResultLabel(Door door) {
    ...
    if (hasCritical) return 'CRITICAL FAIL';
  ...
  static String _doorMaterialLabel(Door door) {
    if (door.material == DoorMaterial.otherCustom && _cleanText(door.customMaterial).isNotEmpty) {
  ...
  static String _doorCertificationStatusLabel(DoorClassification classification) {
  ...
  static bool _isPass(Door door) {
    final hasFailure = door.issues.any(
      (issue) => issue.severity == IssueSeverity.fail || issue.severity == IssueSeverity.criticalFail,
    );
    if (hasFailure) {
      return false;
    }
    return door.result == DoorResult.pass || door.result == DoorResult.advisory;
  }

  static pw.Widget _sectionCard({required String title, required pw.Widget child, pw.Widget? trailing}) {
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          pw.Row(
            ...
            children: [
              pw.Expanded(
                child: pw.Text(_cleanText(title), style: _style(9.0, bold: true, color: _text)),
              ),
  ...
  static pw.Widget _detailGrid(List<List<String>> rows, {required int columns}) {
  ...
  static pw.Widget _detailTile(String label, String value) {
    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: [
        pw.Text(_cleanText(label), style: _style(8.1, bold: true, color: _secondaryText)),
        pw.SizedBox(height: 4),
        pw.Text(_cleanText(value), style: _style(9.5, bold: true, color: _text)),
      ],
    );
  }

  static pw.Widget _statusBadge(String label, PdfColor color, PdfColor background) {
    return pw.Container(
  ...
  static pw.Widget _tableHeaderCell(String text) {
    return pw.Padding(
      ...
      child: pw.Text(_cleanText(text), style: _style(7.6, bold: true, color: _text)),
    );
  }
  ...
  static pw.Widget _tableCell(String text, {PdfColor? color, bool bold = false}) {
    return pw.Padding(
      padding: const pw.EdgeInsets.symmetric(horizontal: 6, vertical: 4),
      child: pw.Text(_cleanText(text), style: _style(7.8, bold: bold, color: color ?? _text)),
    );
  }
  ...
  static pw.Widget _tCellHdr(String text) {
    return pw.Padding(
      ...
      child: pw.Text(_cleanText(text), style: _style(7.2, bold: true, color: _secondaryText)),
    );
  }
  ...
  static pw.Widget _tCell(String text, {bool bold = false}) {
    return pw.Padding(
      padding: const pw.EdgeInsets.symmetric(horizontal: 5, vertical: 3),
      child: pw.Text(_cleanText(text), style: _style(7.7, bold: bold, color: _text)),
    );
  }
      ...
      child: pw.Column(
        ...
        children: lines.map((line) {
          ...
          if (m != null) {
            return pw.RichText(
              text: pw.TextSpan(children: [
                pw.TextSpan(text: '${m.group(1)} - ', style: _style(7.1, bold: true, color: _info)),
                pw.TextSpan(text: _cleanText(m.group(2) ?? ''), style: _style(7.1, color: _text)),
              ]),
            );
          }
  ...
  static pw.Widget _tCellResult(String label, ({PdfColor color, PdfColor background}) style) {
    return pw.Padding(
      ...
      child: pw.Container(
        ...
        child: pw.Text(_cleanText(label), style: _style(6.8, bold: true, color: style.color)),
      ),
    );
  }
  ...
  static String _projectAddress(Survey survey) {
    ...
    final combined = [line1, line2, city, postCode].where((part) => part.isNotEmpty).join(', ');
  ...
  static pw.TextStyle _style(double size, {bool bold = false, required PdfColor color}) {
    return pw.TextStyle(
      fontSize: size,
  ...
  static String _cleanText(String value) {
    ...
    for (final rune in value.runes) {
      ...
      final isControl = codePoint < 0x20 || (codePoint >= 0x7F && codePoint <= 0x9F);
      if (isControl) {
        continue;
      }
  ...
  /// Build replacement information widget for Fire Door module.
  /// Shows \"Full door set replacement required – immediate action recommended\" when needed.
  static pw.Widget? _replacementInformationWidget(Door door, {int? pageNumber}) {
    if (!door.replacementRequired) return null;

    // Format dimensions display based on configuration
    String dimensionText = '';
    final door1w = _cleanText(door.replacementDoor1Width);
    final door1h = _cleanText(door.replacementDoor1Height);
    final door2w = _cleanText(door.replacementDoor2Width);
    final door2h = _cleanText(door.replacementDoor2Height);

    if (door2w.isEmpty && door2h.isEmpty) {
      // Single door
      dimensionText = 'Dimensions: ';
      if (door1w.isNotEmpty || door1h.isNotEmpty) {
        dimensionText += '${door1w.isEmpty ? '—' : door1w} × ${door1h.isEmpty ? '—' : door1h}';
      } else {
        dimensionText += 'Not specified';
      }
    } else {
      // Double or half-and-half door
      dimensionText = 'Door 1: ${door1w.isEmpty ? '—' : door1w} × ${door1h.isEmpty ? '—' : door1h}\\n';
      dimensionText += 'Door 2: ${door2w.isEmpty ? '—' : door2w} × ${door2h.isEmpty ? '—' : door2h}';
    }

    return pw.Container(
      decoration: pw.BoxDecoration(
        border: pw.Border.all(color: _fail, width: 1.5),
        color: _failBg,
        borderRadius: pw.BorderRadius.circular(_cardRadius),
      ),
      ...
      child: pw.Column(
        ...
        children: [
          pw.Text(
            ...
            style: _style(10.0, bold: true, color: _fail),
          ),
          pw.SizedBox(height: 6),
          pw.Text(
            'Full door set replacement required – immediate action recommended',
            style: _style(9.0, color: _fail),
          ),
          pw.SizedBox(height: 6),
          pw.Text(
            dimensionText,
            style: _style(8.8, color: _text),
          ),
          if (pageNumber != null) ...[
            pw.SizedBox(height: 4),
            pw.Text(
